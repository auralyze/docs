...
The `/api` repo exposes a thin REST layer in front of `@auralyze/engine`. All
endpoints live under the same Express app and share JWT/Cerberus middleware.

## Base URL & Auth

- **Base URL** – depends on your deployment (`https://api.auralyze.com` or
  `http://localhost:4001`).
...
Response: `EngineState` as defined in `@auralyze/engine`. Include an auth token
