...
Mixtape uses **microservices** with clear domain boundaries:
...
Mixtape uses **LangGraph** (from LangChain ecosystem) to orchestrate the workflow across services.
...
Mixtape uses **PostgreSQL** for relational data storage with **Prisma** as the ORM.
